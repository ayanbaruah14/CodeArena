...
export default function Room() {
  ...
  const [contestStatus, setContestStatus] = useState("none"); // track live contest state
  ...
  useEffect(() => {
    ...
    // keep contest status badge in sync
    socket.on("contestSetup",   () => setContestStatus("waiting"));
    socket.on("contestStarted", () => setContestStatus("active"));
    socket.on("contestEnded",   () => setContestStatus("ended"));
    socket.on("contestState",   (d) => setContestStatus(d.status));
  ...
  useEffect(() => {
    ...
    API.get(`/rooms/${roomId}/contest`)
      .then(res => setContestStatus(res.data.contest?.status || "none"))
      .catch(() => {});
  }, [roomId]);
  ...
  const isCreator = userId && creator && userId.toString() === creator.toString();

  const contestBtnLabel = () => {
    if (contestStatus === "active")  return { label: "⚡ JOIN BATTLE",   color: "#39ff14", border: "rgba(57,255,20,.3)",  bg: "rgba(57,255,20,.06)"  };
    if (contestStatus === "waiting") return { label: "◈ CONTEST LOBBY",  color: "#00f5ff", border: "rgba(0,245,255,.3)",  bg: "rgba(0,245,255,.06)"  };
    if (contestStatus === "ended")   return { label: "🏆 VIEW RESULTS",  color: "#ffd700", border: "rgba(255,215,0,.3)",  bg: "rgba(255,215,0,.06)"  };
    // none — show differently for host vs member
    if (isCreator)                   return { label: "⚡ CREATE CONTEST", color: "#ff2d78", border: "rgba(255,45,120,.35)", bg: "rgba(255,45,120,.06)" };
    return                                  { label: "◈ CONTEST",        color: "#ff2d78", border: "rgba(255,45,120,.3)",  bg: "rgba(255,45,120,.04)" };
  };

  const btn = contestBtnLabel();
        ...
        <div className="relative z-10 flex flex-col flex-1 p-5 gap-4 max-w-5xl mx-auto w-full">
          ...
          <div className="flex gap-4 flex-1 min-h-0">
            ...
            <div className="w-52 flex-shrink-0 flex flex-col gap-3">
              ...
              {/* ── CONTEST ACTIONS ── */}
              <div className="room-contest-actions">
                <button
                  onClick={() => navigate(`/room/${roomId}/contest`)}
                  className={`room-contest-btn room-contest-btn--${
                    contestStatus === "active"  ? "active"
                  : contestStatus === "waiting" ? "waiting"
                  : contestStatus === "ended"   ? "ended"
                  : "none"
                  }`}
                >
                  {contestStatus === "active" && <span className="room-contest-live-dot" />}
                  {btn.label}
                </button>

                {contestStatus !== "none" && (
                  <div className={`room-contest-status room-contest-status--${contestStatus}`}>
                    {contestStatus === "active"  && "⚡ BATTLE IN PROGRESS"}
                    {contestStatus === "waiting" && "◈ WAITING TO START"}
                    {contestStatus === "ended"   && "🏆 CONTEST FINISHED"}
                  </div>
                )}
  ...
  ⌨ COLLAB EDITOR
                  ...
                  {users.map((u, i) => (
                    <div key={i} className="flex items-center gap-2 px-2 py-1.5"
                      ...
                      <div className="flex items-center justify-center w-6 h-6 rounded-full flex-shrink-0"
                        style={{ border:"1px solid rgba(0,245,255,.3)", background:"rgba(0,245,255,.1)",
                          fontFamily:"'Bebas Neue',sans-serif", fontSize:".6rem", color:"#00f5ff" }}>
                        {u.username?.[0]?.toUpperCase()}
                      </div>
                      <span className="flex-1 truncate"
                        style={{ fontFamily:"'Rajdhani',sans-serif", fontSize:".8rem", fontWeight:600,
                          color: u.userId === userId ? "#00f5ff" : "rgba(255,255,255,.65)" }}>
                        {u.username}
                        {u.userId === userId && (
                      ...
                      {u.userId?.toString() === creator?.toString() && (
              ...
              <div className="p-3"
                ...
                {[
                  ...
                  { label:"HOST",    val: users.find(u => u.userId?.toString() === creator?.toString())?.username || "—" },
                ].map(({ label, val }) => (
            ...
            <div className="flex-1 flex flex-col min-h-0"
                  ...
                  m.system ? (
                    ...
                    <div key={i}
                      ...
                      <div className="flex items-center justify-center w-7 h-7 rounded-full flex-shrink-0 self-end"
                        style={{
                          border: m.userId === userId ? "1px solid rgba(255,45,120,.4)" : "1px solid rgba(0,245,255,.3)",
                          background: m.userId === userId ? "rgba(255,45,120,.1)" : "rgba(0,245,255,.08)",
                      ...
                      <div className="flex flex-col gap-1 max-w-[70%]"
                        ...
                        <div className="px-3 py-2"
                          style={{
                            background: m.userId === userId ? "rgba(255,45,120,.08)" : "rgba(0,245,255,.06)",
                            border: m.userId === userId ? "1px solid rgba(255,45,120,.2)" : "1px solid rgba(0,245,255,.15)",
              ...
              <div className="flex gap-2 p-3"
                ...
                <input
                  ...
                  style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:".62rem", letterSpacing:".06em",
                ...
                <button onClick={sendMessage}
                  ...
                  style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:".55rem",
